...
def get_sec(sec_company):
    base = "http://www.glassdoor.com"
    sec_url = base + sec_company
...
def parse_satisfaction(soup):
    ...
    def _score_sec(soup):
        selector = {'class': 'gdRatingValueBar gdrMidmed'}
...
def parse_suggestions(soup):
    def _suggestions(soup):
        ...
        url = companies_div.find('a')['href']
        res = get_sec(url)   
        return res
    return _suggestions(soup)

def parse(soup, raw=False):

    if soup.findAll('div', {'class': 'sortBar'}):
        print "Glassdoor: Suggestions found, trying the closest ...\n"
        soup = parse_suggestions(soup)
